...
(async function init() {
    await requireAuth();
    document.getElementById('print-date-customers').textContent = new Date().toLocaleString('en-GB');
...
function downloadCustomersExcel() {
    ...
    exportToExcel('eDESK_Customers_' + new Date().toISOString().slice(0, 10) + '.csv', ['Name', 'Phone', 'Purchases', 'Total Spent', 'Outstanding Debt', 'Overdue Debts', 'Status', 'Last Purchase'], rows);
}