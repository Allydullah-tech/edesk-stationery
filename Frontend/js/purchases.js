/**
 * EDESK STATIONERY - Purchases page logic
 * Admin-only. Lets a person add a single received product, or import a
 * whole supplier list from an Excel/CSV file, all funnelling into Stock.
 */
...
async function loadRecentPurchases() {
  ...
  const titleMap = { '': 'Purchases Report', manual: 'Manual Purchases Report', import: 'Imported Purchases Report' };
  document.getElementById('purchasesReportTitle').textContent = titleMap[source];
  document.getElementById('print-date-purchases').textContent = 'Generated ' + new Date().toLocaleString('en-GB');
  ...
  res.data.forEach(p => {
    ...
    distinctProducts.add(p.product_id);
  });
...
function downloadPurchasesExcel() {
  ...
  exportToExcel('eDESK_Purchases' + suffix + '_' + todayStr() + '.csv',
    ['Date', 'Product', 'Unit', 'Qty', 'Buying Price', 'Total Cost', 'Source', 'Recorded By'], rows);
}
...
function setupSingleProductSearch() {
  ...
  input.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); SP_HIGHLIGHT_INDEX = Math.min(SP_HIGHLIGHT_INDEX + 1, SP_MATCHES.length - 1); paintSpHighlight(); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); SP_HIGHLIGHT_INDEX = Math.max(SP_HIGHLIGHT_INDEX - 1, 0); paintSpHighlight(); }
    else if (e.key === 'Enter' && SP_HIGHLIGHT_INDEX >= 0) { e.preventDefault(); selectSingleProduct(SP_MATCHES[SP_HIGHLIGHT_INDEX].id); }
...
function renderSingleSuggestions(query) {
  ...
  list.innerHTML = SP_MATCHES.map((p, i) => `
    <div class="autocomplete-item" data-id="${p.id}">
      ...
      <span class="meta">${p.is_service == 1 ? 'Service' : p.stock_quantity + ' ' + p.unit + ' in stock'}</span>
    </div>`).join('');
...
function quickAddSingleProduct() {
  ...
  openQuickAddModal(name, category, unit, () => { updateMatchHint(name); clearSingleValidation(); });
}
...
/**
 * Small, professional inline validation text (red, no background) shown
 * right under the product name field on the single-purchase form when the
 * person tries to save before the product has actually been added to the
 * Product List.
 */
...
function addBlankImportRow() {
  addImportRow({ name: '', quantity: '', buying_price: '', selling_price: '', category: '', unit: 'pcs' });
  document.getElementById('importPreviewWrap').classList.remove('hidden');
}
...
async function confirmImport() {
  ...
  rows.forEach((tr, i) => {
    ...
    items.push({ name, quantity: qty, buying_price: buying, selling_price: selling, unit, category });
  });
